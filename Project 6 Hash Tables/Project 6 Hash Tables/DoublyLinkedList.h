...
*/
	...
	*/
	...
	/*
	Funtion:		returnNodeATIndex(const int& index)
	Author:			Johnathan Bates
	Description:	Accessor. Takes in an integer(desired index) and traverses the list to the index, then returns the Node or NULL
	Inputs:			Integer
	Outputs:		Template Data
	*/
	T returnNodeATIndex(const int& index)
	{
		int i = 0;
		Node* CurrPos = Head->Next;//uses current position ptr and sets it equal to the beginning of the list
			while (CurrPos->Next != Tail && i != index)//either cycles through the list entirely or stops at the position where the input matches the data, effectively finding the node that was searched for **whatever is being insterted to data needs to have to overload the != operator to acheive this
			{
				CurrPos = CurrPos->Next;//increments the currpos ptr to the next node
				i++;//increments the variable i
			}
			if (i == index)//checks to see if i equals the desired index
			{
				return CurrPos->Data;//returns the data stored in current Node
			}
			else
	...
	/*
	Funtion:		findIndex(const T& input)
	Author :		Johnathan Bates
	Description :	Accessor. Starts at the beginning of the list and cycles through every node until reaching said Node, then returns the index position
	Inputs :		Const Template Data
	Outputs :		Integer
	*/
	int findIndex(const T& input)
	{
		...
		while (p->Next != Tail && p->Data != input)//cycles through the nodes until the end of the list is reached
		{
		...
		if (p->Data == input)
		{
			return i;//returns offset in list
		}
		else
		return NULL;
	}
	...
	*/
	...
	Funtion:		returnTail()
	Author:			Johnathan Bates
	Description:	Accessor. returns the Node just previous of the Tail Node
	Inputs:			None
	Outputs:		Int
	*/
	T returnTail() { return Tail->Prev->Data; }
	/*
	Funtion:		returnHead()
	Author:			Johnathan Bates
	Description:	Accessor. returns the Node just after of the Head Node
	Inputs:			None
	Outputs:		Int
	*/
	T returnHead() { return Head->Next->Data; }//returns the data in the Node just after Head to the caller
	/*
	Funtion:		size_of_List()
	Author:			Johnathan Bates
	Description:	Accessor. returns the size of the list to the caller
	Inputs:			None
	Outputs:		Int
	*/
	...
	Funtion:		countCopies()
	Author:			Johnathan Bates
	Description:	Accessor. Cycles through list and counts how many time an element appears within list.(Specially made for hash table)
	Inputs:			Template data
	Outputs:		Int
	*/
	int countCopies(T data)
	{
		Node* p = Head->Next;//creates a new pointer and sets it at the beginning of the list
		int count = 0;//creates int i and sets it equal to zero. This is used to keep track of where we are in the list
		if (p->Next != NULL)
		{
			while (p->Next != Tail)//cycles through the nodes until the end of the list is reached
			{
				if (p->Data == data)
				{
					count++;//increments the count of the searched item if found
				}
				p = p->Next;//cycling to next Node			
			}
			if (p->Data == data)
